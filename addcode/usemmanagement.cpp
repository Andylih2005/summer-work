...
/*
    people.clear();
    const char * sql="SSELECT id FROM reader";
    sqlite3_stmt *stmt;
    int rc;
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        int outoftime;
        read temp;
        temp.idname=QString((char*)sqlite3_column_text(stmt, 0));*/
    ...
    people.clear();
    const char *sql = "SELECT id FROM reader";
    sqlite3_stmt *stmt;
    ...
    for (size_t i = 0; i < people.size(); ++i) {
        ...
        int borrowtime = 0;
        int ownmoney = 0;
        int current_date = getcurrenttime2();
        ...
        while (sqlite3_step(record_stmt) == SQLITE_ROW)
        {
            ...
            // 计算借书时间（天数）
            int borrow_duration = outtime-intime;
            qDebug()<<borrow_duration;
            if (borrow_duration > 14) {
                ownmoney += borrow_duration - 14;
            }
        }
        ...
        qDebug() << borrowtime;
        temp.own = QString::number(ownmoney);
        qDebug() << ownmoney;
