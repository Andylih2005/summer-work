...
void kmp(std::string t,int *next)
{
    next[0]=-1;
    int k =-1,j=0;
    int len=t.size();
...
bool correct(std::string in,std::string bookname,int *next)
{
    int len1=in.size();
    int len2=bookname.size();
    int i=0,j=0;
    while((i<len2)&&(j<len1))
    {
        if (j==-1||bookname[i]==in[j])
        {
            i++;
            j++;
        }
...
void findbook::paintEvent(QPaintEvent *event)
{
...
void findbook::mousePressEvent(QMouseEvent *event)
{
...
void findbook::mouseMoveEvent(QMouseEvent *event)
{
...
void findbook::on_find_button_clicked()
{
    ...
    else
    {
        if (TypeName=="")
        {
            int *next=new int[temp1.size()];
            kmp(temp1,next);
            qDebug()<<"typename empty";
            rc = sqlite3_prepare_v2(db, "SELECT name,author,IBSN FROM book", -1, &stmt, NULL);
            ...
            {
                std::string tt=tempsave[i].name.toStdString();
                if(correct(temp1,tt,next))
                {
                    savedata.push_back(tempsave[i]);
                }
            }
        }
        else
...
const char * getcredit1(char * id)
{
    static char buf[BUFSIZ]={0};
    sqlite3_snprintf(BUFSIZ,buf,"SELECT credit FROM reader WHERE id='%s'",id);
    return buf;
}
void findbook::Borrowing(QString bookname)
{
    ...
    if (reply==QMessageBox::Yes)
        ...
        int rc;
        sqlite3_stmt* stmt;
        const char * cc=getcredit1(nowaccount);
        rc=sqlite3_prepare_v2(db,cc,-1,&stmt,NULL);
...
void findbook::on_btnClose_clicked()
{
    this->close();
}
